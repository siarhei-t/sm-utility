...
modbus::Exceptions ModbusServer::readRegister(std::uint8_t data[]) const
{
    ...
    const std::uint8_t index_crc = modbus::rw_reg_pdu_suze + modbus::address_size + server_config.msg_start_size;
    const std::uint8_t index_address = server_config.data_start_idx;
    const std::uint8_t index_quantity = server_config.data_start_idx + 2;
...
{
    const std::uint8_t data_length  = modbus::write_file_pdu_size + modbus::address_size;
    const std::uint8_t index_crc = modbus::write_file_pdu_size + modbus::address_size + server_config.msg_start_size;

    std::uint16_t received_crc = extractHalfWord(&data[index_crc]);
    std::uint16_t actual_crc  = CRC16(&data[server_config.msg_start_size],data_length);
    if(received_crc == actual_crc)
    {
        FileService file_service(extractHalfWord(&data[server_config.data_start_idx + 2]),
                               extractHalfWord(&data[server_config.data_start_idx + 4]),
                                  extractHalfWord(&data[server_config.data_start_idx + 6]));
        
        uint8_t byte_counter = data[server_config.data_start_idx];
        uint8_t reference_type = data[server_config.data_start_idx + 1];
        ...
        {
            if(writeFile(file_service,&data[server_config.data_start_idx + 8]))
            {
                return modbus::Exceptions::no_exception;
            }
            else
...
{
    const std::uint8_t data_length  = modbus::read_file_pdu_size + modbus::address_size;
    const std::uint8_t index_crc = modbus::read_file_pdu_size + modbus::address_size + server_config.msg_start_size;
