...
int main(int argc, char* argv[])
{
    ...
    */
    ...
    try
    {
        ...
        int address = std::stoi(*opts.address);
        if (address > modbus::max_rtu_address)
        {
            std::cout << "out of range address passed, exit...\n";
            return 0;
        }
        else if (address < modbus::min_rtu_address)
        {
        ...
        auto error_code = client.start(*opts.device_path);
        if (error_code)
        {
        ...
        client.setAddress(static_cast<std::uint8_t>(address));
