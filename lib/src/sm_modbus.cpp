...
static void insertHalfWord(std::vector<std::uint8_t>& arr,
...
Sizes get_sizes(modbus::ModbusMode mode)
{
    switch (mode)
    {
        ...
        case modbus::ModbusMode::ascii:
            return Sizes{modbus::ascii_start_size, modbus::ascii_stop_size,
                         modbus::ascii_adu_size};

        case modbus::ModbusMode::rtu:
            return Sizes{modbus::rtu_start_size, modbus::rtu_stop_size,
                         modbus::rtu_adu_size};
    }
    return {};
}
...
ModbusClient::msgCustom(const std::uint8_t addr, const std::uint8_t func,
...
std::vector<std::uint8_t>& ModbusClient::msgWriteFileRecord(
    const std::uint8_t addr, const std::uint16_t file_id,
    const std::uint16_t record_id, const std::vector<std::uint8_t>& record_data)
{
    const std::uint8_t rec_data_length =
        record_data.size() + 7; // 7 additional bytes for record data
    const std::uint16_t record_length =
        record_data.size() / 2; // record splited into half words
    ...
    createMessage(addr, static_cast<std::uint8_t>(FunctionCodes::write_file),
                  record);
    return buffer;
}

std::vector<std::uint8_t>& ModbusClient::msgReadFileRecord(
    const std::uint8_t addr, const std::uint16_t file_id,
    const std::uint16_t record_id, const std::uint16_t length)
{
    std::vector<uint8_t> record;
    record.insert(record.end(),
                  {0x07, 0x06}); // 7 bytes in this message (support for reading
                                 // only one record per message)
    insertHalfWord(record, file_id);
    insertHalfWord(record, record_id);
    insertHalfWord(record, length);
    createMessage(addr, static_cast<std::uint8_t>(FunctionCodes::read_file),
                  record);
    return buffer;
}

std::vector<std::uint8_t>&
ModbusClient::msgWriteRegister(const std::uint8_t addr, const std::uint16_t reg,
                               const std::uint16_t value)
{
    ...
    createMessage(
        addr, static_cast<std::uint8_t>(FunctionCodes::write_register), data);
    return buffer;
}

std::vector<std::uint8_t>&
ModbusClient::msgReadRegisters(const std::uint8_t addr, const std::uint16_t reg,
                               const std::uint16_t quantity)
{
    ...
    createMessage(
        addr, static_cast<std::uint8_t>(FunctionCodes::read_registers), data);
    return buffer;
}
...
bool ModbusClient::isChecksumValid(const std::vector<std::uint8_t>& data)
{
    ...
    {
        message.insert(message.end(), data.begin() + sizes.start_seq_size,
                       data.end() - sizes.stop_seq_size - crc_size);
...
void ModbusClient::extractData(const std::vector<std::uint8_t>& data,
                               std::vector<std::uint8_t>& message)
{
    Sizes sizes = get_sizes(mode);
    message.insert(message.end(), data.begin() + sizes.start_seq_size,
                   data.end() - sizes.stop_seq_size);
}
...
void ModbusClient::createMessage(const std::uint8_t addr,
                                 ...
                                 const std::vector<std::uint8_t>& data)
{
    ...
    {
        case ModbusMode::rtu:
            buffer.insert(buffer.begin(), &rtu_start_end[0],
                          &rtu_start_end[0] + sizeof(rtu_start_end));
            buffer.insert(buffer.end(), &rtu_start_end[0],
                          &rtu_start_end[0] + sizeof(rtu_start_end));
            break;

        case ModbusMode::ascii:
            buffer.insert(buffer.begin(), &ascii_start[0],
