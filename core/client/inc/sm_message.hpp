...
public:
    ...
    std::vector<std::uint8_t>& msgWriteFileRecord(const std::uint8_t addr, const std::uint16_t file_id, const std::uint16_t record_id,
    ...
    /// @return reference to vector with created message
    std::vector<std::uint8_t>& msgReadFileRecord(const std::uint8_t addr, const std::uint16_t file_id, const std::uint16_t record_id,
